  ...
  return (
    <main className="flex-grow pt-[120px] pb-24 px-6 md:px-12 w-full max-w-[1000px] mx-auto">
      ...
      <div className="relative border-l border-surface-container-highest ml-4 md:ml-6 pl-8 md:pl-12 space-y-16">
        {experiences.map((exp) => {
          const colorClass = exp.colorClass || 'primary';
          const isSecondary = colorClass === 'secondary';
          const isNeutral = colorClass === 'surface-variant';
          
          const accentClass = isSecondary ? 'text-secondary-container' : (isNeutral ? 'text-on-surface-variant' : 'text-primary-container');
          const borderHoverClass = isSecondary ? 'group-hover:border-secondary-container' : (isNeutral ? 'group-hover:border-surface-variant' : 'group-hover:border-primary-container');
          const dotClass = isSecondary ? 'bg-secondary-container' : (isNeutral ? 'bg-surface-variant' : 'bg-primary-container');

          return (
            <div key={exp.id} className="relative group">
              {/* Timeline Node */}
              <div className={`absolute -left-[41px] md:-left-[57px] top-0 w-8 h-8 rounded-full bg-surface border-2 border-surface-container-highest flex items-center justify-center ${borderHoverClass} transition-colors duration-300 z-10`}>
                <div className={`w-2 h-2 rounded-full ${dotClass} glow-border-sm`}></div>
              </div>
                ...
                <header className="flex flex-col md:flex-row md:justify-between md:items-start mb-6 gap-4">
                  <div>
                    <h2 className="font-headline-md text-headline-md text-on-surface mb-1 flex items-center gap-2">
                      <span className={`material-symbols-outlined ${accentClass}`}>{exp.icon}</span>
                      {exp.title}
                    </h2>
